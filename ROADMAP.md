...
| **Saint Petersburg** | **in progress** (game 4, 1st edition): **SP0–SP2 shipped** — registered, viewable fourth game with redacting `viewFor` (hands *and* rubles secret) + the 116-card deck data (SP0), the playable **phase spine** — `BUY`/`PASS`, turn order, cumulative cost reductions (min 1 ruble), all-pass phase close with scoring + refill, interactive board (SP1), and the **full round loop** — trading-phase frame (no scoring), round transition (discard lower, slide upper→lower, refill workers, rotate all four markers left) with the pg. 8 "no cards taken → skip the refill" special case, marker chips + rollover feed (SP2); SP3–SP9 (hand, trading, specials, end scoring, bot) still to come | [`engine/src/games/stpetersburg/ROADMAP.md`](engine/src/games/stpetersburg/ROADMAP.md) |
