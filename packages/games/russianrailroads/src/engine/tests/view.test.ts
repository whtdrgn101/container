...
function withSecret(count = 3): RussianRailroadsState {
  ...
  return {
    ...state,
    players: state.players.map((p) => (p.id === 'p2' ? { ...p, endBonus: END_BONUS_CARDS[0]! } : p)),
  };
}
...
describe('viewFor', () => {
  it('shows the owner their own end-bonus card and hides an opponent’s', () => {
    ...
    expect(p2.endBonus).toBeNull(); // redacted
    expect(p2.endBonusHeld).toBe(1); // but the count is public

    const asP2 = viewFor(state, 'p2');
    expect(asP2.players.find((p) => p.id === 'p2')!.endBonus).toEqual(END_BONUS_CARDS[0]);
  ...
  it('honours a seat list and a spectator (null) viewer', () => {
    ...
    expect(asBoth.players.find((p) => p.id === 'p2')!.endBonus).toEqual(END_BONUS_CARDS[0]);

    const asSpectator = viewFor(state, null);
    expect(asSpectator.players.find((p) => p.id === 'p2')!.endBonus).toBeNull();
  ...
  it('reveals everything once the game has ended', () => {
    ...
    expect(asP1.players.find((p) => p.id === 'p2')!.endBonus).toEqual(END_BONUS_CARDS[0]);
  });
  ...
  it('survives a JSON round-trip with no secret leaking (the wire test)', () => {
    ...
    const players = wire['players'] as { id: string; endBonus: unknown }[];
    expect(players.find((p) => p.id === 'p2')!.endBonus).toBeNull();
