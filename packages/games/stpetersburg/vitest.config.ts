...
/**
 * Saint Petersburg is the Track D legacy-migration **phase 4** retrofit: moved out of the shared `engine/`
 * + `bot/` packages into its own in-workspace game package, following the RR precedent and the Can't
 * Stop / Stone Age phases. This config carries **both** coverage gates the game used to answer to — the
 * platform-wide engine 100% gate (`engine/vitest.config.ts`) and the bot 90% gate (`bot/vitest.config.ts`)
 * no longer reach this game (it lives outside those packages), so this file is what keeps that discipline
 * honest.
 *
 ...
 * bot and run here.
 *
 * `@game-hub/kernel` ships TypeScript source (the engine/bot import it and its `./bot` subpath), so Vitest
 * must transform it across the workspace boundary.
 */
