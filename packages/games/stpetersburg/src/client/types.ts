...
/**
 * The `GameClient` seam, bound for Saint Petersburg (Track D — legacy-migration phase 4).
 *
 ...
 * binding `ui/src/games/types.ts` makes for the in-repo games, and the RR/Can't Stop/Stone Age precedent's
 * `src/client/types.ts`. Because this package lives outside `ui/src`, it reaches the shell's transport
 * types (and the shared board components) through the UI's `@` alias — a Track D finding logged in the
 * ROADMAP: the shell isn't yet a package a game can depend on, so the client subpath still couples to
 * `ui/src` at build time.
 */
