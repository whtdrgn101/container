...
describe('displacement helpers (pg. 7–8)', () => {
  ...
  it('a blue trading card displaces ANY building; an orange one ANY aristocrat (pg. 7)', () => {
    ...
    expect(
      legalDisplaceTargets({ playArea: area({ building: [market, customs] }) }, mariinskij).map((c) => c.id),
    ).toEqual(['mk', 'ch']);
    expect(legalDisplaceTargets({ playArea: area({ aristocrat: [scribe, clerk] }) }, abbot).map((c) => c.id)).toEqual([
      'sc',
      'ck',
    ]);
  });
  ...
  it('a green trading card matches by ware symbol, and the Czar by anything (pg. 8)', () => {
    ...
    const targets = legalDisplaceTargets(
      { playArea: area({ worker: [lumberjack, goldMiner, czar] }) },
      carpenterWorkshop,
    );
  ...
  it('validateDisplacement returns the target or throws INVALID_DISPLACE_TARGET', () => {
    expect(validateDisplacement({ playArea: area({ worker: [lumberjack] }) }, carpenterWorkshop, 'lj')).toBe(
      lumberjack,
    );
    expect(() =>
      validateDisplacement({ playArea: area({ worker: [lumberjack] }) }, carpenterWorkshop, 'missing'),
    ).toThrow(/INVALID_DISPLACE_TARGET|displaced/);
  });
