...
engine (`src/engine/`), backend module (`src/module/`), UI client
(`src/client/`), and its bot (`src/bot/`) — all in this package (`@game-hub/game-container`). Platform/engine-wide work (the `GameModule` /
...
- ✅ **Refactor R1 — Engine modularization:** split the monolith into `core/` / `internal/` / `actions/`
  with barrels + per-piece tests. (Superseded by the C3 per-game reorg and then Track D — Container now
  lives under this package's `src/engine/`; see [`docs/game-creation.md`](../../../docs/game-creation.md) §2.)
...
Container's bot lives in this package's `src/bot/` (over `@game-hub/kernel/bot`); the per-game bot
reorganization (so a second game can have its own bot) shipped, and Track D later moved every bot into its
game package — a platform item in the top-level roadmap.
...
- **Bots live in each game package's `src/bot/`** (over `@game-hub/kernel/bot`) with their **own ~90%
  coverage gate** so heuristic tuning doesn't fight the engine's 100% bar. **Engine = rules, bot =
  opinions.** No bot code
  enters `engine/`.
