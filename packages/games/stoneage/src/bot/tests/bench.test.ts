...
describe('stoneage strength bench', () => {
  // A self-bench (candidate === baseline) over a handful of seeded games — assert the harness runs a real
  // game to its end and reports a sane rate, not ≈50% (far too noisy on this few games to assert). The
  // real strength claim lives in benchmark.test.ts (live vs. frozen legacy policy).
