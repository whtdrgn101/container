...
function pending(
  ...
  const base = withPlacements(
    { [place]: { p1: dice.length }, ...opts.placed },
...
describe('takeGather (take — step 2)', () => {
  it('takes the yield, returns the people, and clears the pending roll', () => {
    ...
    expect(next.log.at(-1)).toMatchObject({
      type: 'TAKE',
      payload: { place: 'forest', amount: 3, kind: 'wood', boost: 0 },
    });
  });
  ...
  it('adds the chosen tools to the total and spends them for the round', () => {
    ...
    const next = takeGather(
      pending('forest', [1, 1], { p1: { tools: [2, 3], toolsUsed: [false, false] } }),
      'p1',
      [0, 1],
    );
  ...
  it('advances the turn like a gather: stays, passes, or ends the action phase', () => {
    ...
    expect(
      takeGather(pending('forest', [1, 1, 1], { placed: { quarry: { p1: 2 } } }), 'p1', []).activePlayerIndex,
    ).toBe(0);
    // Passes: p2 has a place to work.
    expect(
      takeGather(pending('forest', [1, 1, 1], { placed: { clayPit: { p2: 2 } } }), 'p1', []).activePlayerIndex,
    ).toBe(1);
  ...
  it('rejects a take with no pending roll or a bad tool selection', () => {
    ...
    expectError(
      () => takeGather(pending('forest', [1], { p1: { tools: [1], toolsUsed: [false] } }), 'p1', [5]),
      'INVALID_TAKE',
    ); // out of range
    expectError(
      () => takeGather(pending('forest', [1], { p1: { tools: [1, 2], toolsUsed: [true, false] } }), 'p1', [0]),
      'INVALID_TAKE',
    ); // already used
    expectError(
      () => takeGather(pending('forest', [1], { p1: { tools: [1], toolsUsed: [false] } }), 'p1', [0, 0]),
      'INVALID_TAKE',
    ); // duplicate
    expectError(
      () => takeGather(pending('forest', [1], { p1: { tools: [1], toolsUsed: [false] } }), 'p1', [-1]),
      'INVALID_TAKE',
    ); // negative
  });
});
