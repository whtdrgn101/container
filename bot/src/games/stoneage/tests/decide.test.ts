...
import { applyAction, createGame, viewFor } from '@game-hub/engine/stoneage';
...
describe('decide', () => {
  ...
  it('rolls a gather (needing injected dice), then takes the pending roll', () => {
    const acting: StoneAgeState = { ...base(), phase: 'actions', activePlayerIndex: 0, placements: { ...base().placements, forest: { p1: 2 } } };
  ...
  it('uses a non-dice place', () => {
    const acting: StoneAgeState = { ...base(), phase: 'actions', activePlayerIndex: 0, placements: { ...base().placements, field: { p1: 1 } } };
    expect(decide(view(acting), 'p1')).toEqual({ type: 'USE', place: 'field' });
  });

  it('declines a building when too poor, and buys one it can afford', () => {
    const s = base();
    const onBuilding: StoneAgeState = { ...s, phase: 'actions', activePlayerIndex: 0, placements: { ...s.placements, building1: { p1: 1 } } };
    ...
    const rich = { ...onBuilding, players: onBuilding.players.map((p, i) => (i === 0 ? { ...p, resources: { wood: 2, brick: 1, stone: 0, gold: 0 } } : p)) };
